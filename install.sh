...
if [ -z "$( grep -e "typo-birman-ua" /usr/share/X11/xkb/symbols/ua )" ]; then
    sudo bash -c "cat $BASE/symbols/typo-birman-ua >> /usr/share/X11/xkb/symbols/ua"
fi

# Edit /usr/share/X11/xkb/rules/evdev.lst

sudo sed -i -E 's/\s*typo-birman.*//g' /usr/share/X11/xkb/rules/evdev.lst
sudo sed -i -E 's/(! variant)/\1\n  typo-birman-en         us: English (Typographic by Ilya Birman)\n  typo-birman-ru         ru: Russian (Typographic by Ilya Birman)\n  typo-birman-ua         ua: Ukrainian (Typographic by Ilya Birman)/g' /usr/share/X11/xkb/rules/evdev.lst

# Edit /usr/share/X11/xkb/rules/evdev.xml

sudo "$BASE/helper/xmladd.py" /usr/share/X11/xkb/rules/evdev.xml "$BASE/rules/variant_en" "$BASE/rules/variant_ru" "$BASE/rules/variant_ua" /tmp/evdev.xml
